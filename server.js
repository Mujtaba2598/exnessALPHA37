...
console.log('🕋 FINAL 100% HALAL EXNESS TRADING BOT');
console.log('📦 Version: 32.0.0 - FINAL');
console.log('✅ REAL EXNESS BALANCE');
console.log('✅ MULTIPLE CONCURRENT TRADES (UNLIMITED)');
console.log('✅ NO LEVERAGE - 1:1 SPOT TRADING');
console.log('✅ 100% HALAL - NO RIBA, NO GHARAR, NO MAYSIR');
console.log('✅ NO SIMULATION - REAL DATA ONLY');
    ...
    try {
        ...
        if (!ticker) {
            return res.status(500).json({ 
                success: false, 
                message: 'TickerAll not initialized. Please check API key.' 
            });
        }
        ...
        if (!accountId) {
            ...
            return res.status(401).json({ 
                success: false, 
                message: `Could not connect to Exness. Please check:\n1. MT5 Login (number only)\n2. MT5 Password\n3. Server name\n\nError: ${lastError || 'Unknown error'}`
            });
        }
        ...
        if (result.balance === null) {
            return res.status(401).json({ 
                success: false, 
                message: `Connected but could not fetch balance: ${result.error || 'Unknown error'}`
            });
        }
        ...
        res.status(401).json({ 
            success: false, 
            message: error.message || 'Connection failed. Please check your credentials and try again.'
        });
    }
});
...
app.post('/api/connect-exness', authenticate, async (req, res) => {
    try {
        ...
        if (!user || !user.tickerallSessionId) {
            return res.status(400).json({ success: false, message: 'No credentials saved. Please connect first.' });
        }
        ...
        if (result.balance === null) {
            return res.status(401).json({ 
                success: false, 
                message: `Could not fetch balance: ${result.error || 'Unknown error'}`
            });
        }
...
// ==================== FINAL TRADING ENGINE - MULTIPLE CONCURRENT TRADES ====================
const engines = {};
...
class HalalTradingEngine {
    ...
    async forceOpenTrade() {
        if (!this.isActive || this.hasOpenedFirstTrade) return;
    ...
    async executeTrade(symbol, side, aiDecision) {
        try {
            ...
            const positionSize = Math.min(balance / 20, balance * 0.1);
...
app.post('/api/start-trading', authenticate, async (req, res) => {
    try {
        ...
        res.json({
            success: true,
            sessionId,
            message: `🕋 HALAL AI TRADING STARTED! Investment: $${investmentAmount} | Target: $${targetProfit} | NO LEVERAGE - 1:1 Spot Trading | 🤖 AI analyzes EVERY 2 SECONDS | 📊 UP TO 100 CONCURRENT TRADES!`
        });
    } catch (error) {
...
app.listen(PORT, '0.0.0.0', () => {
    console.log(`\n🕋 FINAL 100% HALAL EXNESS TRADING BOT`);
    ...
    console.log(`✅ 📊 UNLIMITED CONCURRENT TRADES (50-100+)`);
